...
internal class Cassette.IndicatorBin : Gtk.Widget, Gtk.Buildable {
    ...
    void update_description () {
        ...
        if (_needs_attention) {
            needs_attention_desc = dpgettext2 (
                "libadwaita",
                "view switcher button badge",
                "Attention requested."
            );
        }

        if (_badge_number > 999) {
            badge_desc = dpgettext2 (
                "libadwaita",
                "view switcher button badge",
                "Has a badge: more than 999."
            );
